class DonationCreator
  ...
  def run
    ...
    Rails.logger.info "Creating AK donation with params: #{params[:params].attributes.inspect}"
    response = client.create_donation(params[:params])
    Rails.logger.info "Response from AK create_donation is: #{response.attributes.inspect}"
    if !response.success?
